tufteBar = function (dat, x, y=NULL, classify = NULL,
  ...
  # Dodge identity bar plot
    ...
    barplot = ggplot(dat, aes_string(x = x, y = y, fill = classify, width = width)) + 
      geom_bar(stat="summary", fun.y = summary, position = position_dodge()) +
      geom_bar(stat="summary", fun.y = summary, position = position_dodge(), color = "white", show_guide=FALSE)
  ...
  # Simple identity bar plot
    ...
    barplot = ggplot(dat, aes_string(x = x, y = y, width=0.6)) + geom_bar(stat="summary", fun.y = summary)
  }
